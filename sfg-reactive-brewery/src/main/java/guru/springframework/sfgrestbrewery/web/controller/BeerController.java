...
public class BeerController {
    ...
    public ResponseEntity<Void> saveNewBeer(@RequestBody @Validated BeerDto beerDto){
        ...
        final var atomicIntegerBeerId = new AtomicInteger();

         beerService.saveNewBeer(beerDto)
                 .map(BeerDto::getId)
                 .subscribe(atomicIntegerBeerId::set);
    ...
    public ResponseEntity<Void> updateBeerById(@PathVariable("beerId") Integer beerId, @RequestBody @Validated BeerDto beerDto) {
        ...
        beerService.updateBeer(beerId, beerDto)
                .map(BeerDto::getId)
                .map(Objects::nonNull)
                .subscribe(atomicBoolean::set);

        return atomicBoolean.get() ? ResponseEntity.noContent().build() : ResponseEntity.notFound().build();
    }
